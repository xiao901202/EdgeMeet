...
# 工具/輔助
# ===============================
def _canonicalize_bytes_to_wav16k_mono(
    contents: bytes, out_wav: Path, src_ext: str
) -> Path:
    """
    ...
    audio = (
        ...
        .set_sample_width(TARGET_BITS // 8)
    )
...
def check_audio_volume(wav_path: Path) -> float:
    ...
    try:
        with wave.open(str(wav_path), "rb") as wf:
            ...
            audio_data = (
                np.frombuffer(frames, dtype=np.int16).astype(np.float32) / 32767.0
            )
...
def _init_json_files(
    folder: Path, total_estimated_segments: int = 0
) -> Tuple[Path, Path]:
    ...
    if volume < VOLUME_THRESHOLD:
        ...
        try:
            if whisper_app:
                ...
                # 模擬模式
                text = (
                    f"（模擬）第 {idx:03d} 段的轉錄文字（{start:.1f}s ~ {end:.1f}s）。"
                )
...
async def generate_batch_summary(
    segments: List[Dict[str, Any]], batch_start_idx: int
) -> str:
    """為一個批次（3段或剩餘段落）生成摘要"""
    if not kuwa_client:
    ...
    for seg in segments:
        ...
        if (
            text
            and text != PROCESSING_TEXT
            and not text.startswith("轉錄失敗")
            and not text.startswith("（模擬）")
        ):
    ...
    try:
        ...
        summary_result = ""
        print(
            f"📝 生成第 {batch_start_idx}-{batch_start_idx + len(segments) - 1} 段的批次摘要..."
        )
...
async def generate_overall_summary(
    all_segments: List[Dict[str, Any]], base_name: str
) -> str:
    """基於批次摘要代表段落生成整體摘要"""
    if not kuwa_client:
    ...
    for seg in all_segments:
        ...
        if (
            ...
            and summary
            and summary != PROCESSING_SUMMARY
            and summary.startswith("index ")
            and "的摘要" in summary
        ):
...
請根據以下內容生成整體摘要：
        ...
        overall_summary = ""
        print(
            f"📋 生成 {base_name} 的整體摘要（基於 {len(batch_summaries)} 個批次摘要）..."
        )
        async for chunk in kuwa_client.chat_complete(messages=message, streaming=True):
...
def _update_batch_summary_in_json(
    tr_path: Path, batch_segments: List[Dict[str, Any]], batch_summary: str
):
...
def _create_summary_json(
    folder: Path, all_segments: List[Dict[str, Any]], overall_summary: str
):
    ...
    for seg in all_segments:
        ...
        sm_data["per_segment"].append({"index": idx, "summary": segment_summary})
    ...
    print(
        f"✅ 新格式 summary.json 已建立，包含 {len(sm_data['per_segment'])} 個段落摘要"
    )
...
def _stitch_stream_chunks_to_base(base_name: str) -> Path:
    ...
    merged = (
        merged.set_frame_rate(TARGET_SR)
        .set_channels(TARGET_CH)
        .set_sample_width(TARGET_BITS // 8)
    )
...
async def transcribe(file: UploadFile = File(...)):
    ...
    full_wav = folder / FULL_WAV
    _canonicalize_bytes_to_wav16k_mono(
        contents, full_wav, os.path.splitext(orig_name)[1]
    )
    ...
    for start_ms in range(0, max(total_ms, 1), step_ms):
        ...
        # 每3段或最後一批，生成批次摘要
        if len(pending_for_summary) >= SUMMARY_BATCH_SIZE or (
            start_ms + chunk_ms >= total_ms
        ):
            batch_start_idx = pending_for_summary[0]["index"]
            batch_end_idx = pending_for_summary[-1]["index"]

            # 生成批次摘要
            batch_summary_text = await generate_batch_summary(
                pending_for_summary, batch_start_idx
            )

            # 更新批次中所有段落的摘要
            _update_batch_summary_in_json(
                tr_path, pending_for_summary, batch_summary_text
            )
...
async def ingest_chunk(
    base_name: str = Query(...), index: int = Query(...), file: UploadFile = File(...)
):
    folder = _ensure_folder(base_name)
    ...
    _canonicalize_bytes_to_wav16k_mono(
        contents, out_wav, os.path.splitext(file.filename)[1]
    )
    ...
    if len(state["pending_segments"]) >= SUMMARY_BATCH_SIZE:
        ...
        # 生成批次摘要
        batch_summary_text = await generate_batch_summary(
            batch_segments, batch_start_idx
        )
        ...
        print(
            f"✅ 串流第 {batch_start_idx}-{batch_end_idx} 段批次完成（轉錄+摘要一起寫入）"
        )
...
async def finalize_stream(base_name: str):
    ...
    if state["pending_segments"]:
        ...
        # 生成最後一批的摘要
        batch_summary_text = await generate_batch_summary(
            state["pending_segments"], batch_start_idx
        )
        ...
        print(
            f"✅ 最終批次第 {batch_start_idx}-{batch_end_idx} 段完成（轉錄+摘要一起寫入，共 {len(state['pending_segments'])} 段）"
        )
...
async def segment_at(base_name: str = Query(...), t: float = Query(..., ge=0.0)):
    ...
    if not tr_path.exists():
        raise HTTPException(
            status_code=404, detail="Transcript not found. Please transcribe first."
        )
...
async def get_summary(base_name: str = Query(...)):
    ...
    if not sp.exists():
        raise HTTPException(
            status_code=404, detail="Summary not found. Please transcribe first."
        )
...
async def model_status():
    ...
    return {
        ...
        "status": (
            "ready"
            if (whisper_app and kuwa_client)
            else "partial" if (whisper_app or kuwa_client) else "simulation_mode"
        ),
